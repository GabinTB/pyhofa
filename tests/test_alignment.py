...
def test_align_loadings_resolves_permutation_and_sign_without_rotation() -> None:
    ...
    aligned = align_loadings(reference, candidate, allow_rotation=False)

    np.testing.assert_allclose(aligned, reference, atol=1e-12)


def test_align_loadings_resolves_orthogonal_rotation_by_default() -> None:
