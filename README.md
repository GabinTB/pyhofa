...
Fix the factor count, non-Gaussian/Gaussian split and selected cumulant order
on an initial training block; do not re-select them independently in every
short rolling window. Refit each window with that fixed estimator, align its
loadings against the preceding fit, then project observations with that
window's frozen preprocessing statistics. For a third-order training choice:
